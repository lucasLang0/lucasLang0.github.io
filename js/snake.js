...
// weird bug where sometimes loss happens after turning near wall but before collision, will look into further.

// maybe swap player1/player 2 controls so start positions align with where controls are in relation to eachother.
// maybe random start locations.
...
function drawGame() {
    ...
    if (isTwoPlayerMode) {
        snake2.forEach((segment, index) => {
            if (index === 0) {
                ctx.fillStyle = '#a10505ff'; // Red head
            } else {
                ctx.fillStyle = '#f53434ff'; // Red body
            }
            ctx.fillRect(segment.x * gridSize, segment.y * gridSize, gridSize - 2, gridSize - 2);
        });
    }
